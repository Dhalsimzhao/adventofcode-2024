...
function parser (input: Input) {
  const rows: number[][] = input.trim().split('\n').map(row=> row.split(/\s+/).map(s => +s))
  const list1 = [], list2 = []
  rows.forEach(row => {
    list1.push(row[0])
    list2.push(row[1])
...
export function solution1 (input: Input) {
  let [list1, list2] = parser(input)
  list1 = sort(list1)
  list2 = sort(list2)
...
export function solution2(input: Input) {
  let [list1, list2] = parser(input)
  const memo = new Map()
  let result = 0
  list1.forEach(e => {
